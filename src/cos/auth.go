...
func newAuthorization(secretID, secretKey string, req *http.Request, authTime *AuthTime) string {
	...
	formatString := genFormatString(req.Method, (*req.URL).Path, formatParameters, formatHeaders)
...
func NewAuthorizationSimple(
	...
	authTime := NewAuthTime(expire)
	signTime := authTime.signString()
	keyTime := authTime.keyString()
	signKey := calSignKey(secretKey, keyTime)

	formatHeaders, signedHeaderList := genFormatHeaders(http.Header{})
	formatParameters, signedParameterList := genFormatParameters(url.Values{})
	formatString := genFormatString(method, pathName, formatParameters, formatHeaders)
...
func genFormatString(method string, path string, formatParameters, formatHeaders string) string {
